  ...
  void flush() {
    ...
    if (ShareLog_.back().Fd.size() >= ShareLogFileSizeLimit_) {
      ...
      while (ShareLog_.size() > 1 && ShareLog_.front().LastId < aggregatedShareId) {
        LOG_F(INFO, "remove old share log file %s", ShareLog_.front().Path.string().c_str());
...
private:
  void replayShares(CShareLogFile &file) {
    if (isDebugBackend())
      LOG_F(1, "%s: Replaying shares from file %s", BackendName_.c_str(), file.Path.string().c_str());
